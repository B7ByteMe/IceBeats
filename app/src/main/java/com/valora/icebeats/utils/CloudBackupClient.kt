    ...
    suspend fun checkBackupExists(email: String): Boolean =
        withContext(Dispatchers.IO) {
            runCatching {
                ...
                val fileName = "icebeats/backups/$folder/icebeats_backup.backup"
                val request =
                    Request
                        .Builder()
        ...
        withContext(Dispatchers.IO) {
            runCatching {
                ...
                val detailsRequest = Request.Builder()
                    .url("$BASE_URL/write?file=icebeats/backups/$folder/details.json")
                ...
                val backupRequest = Request.Builder()
                    .url("$BASE_URL/upload?file=icebeats/backups/$folder/icebeats_backup.backup")
                    .addHeader("X-API-Key", API_KEY)
