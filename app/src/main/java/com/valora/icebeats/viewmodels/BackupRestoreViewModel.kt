...
import com.valora.icebeats.db.InternalDatabase
...
import com.valora.icebeats.extensions.div
import com.valora.icebeats.extensions.tryOrNull
import com.valora.icebeats.extensions.zipInputStream
import com.valora.icebeats.extensions.zipOutputStream
import com.valora.icebeats.playback.MusicService
import com.valora.icebeats.playback.MusicService.Companion.PERSISTENT_QUEUE_FILE
...
class BackupRestoreViewModel @Inject constructor(
        ...
        runCatching {
            context.applicationContext.contentResolver.openOutputStream(uri)?.use {
                it.buffered().zipOutputStream().use { outputStream ->
                    ...
                    if (parentFile != null) {
                        val statsPrefsFile = parentFile / "shared_prefs" / "icebeats_global_stats.xml"
                        if (statsPrefsFile.exists()) {
                            statsPrefsFile.inputStream().buffered().use { inputStream ->
                                outputStream.putNextEntry(ZipEntry("icebeats_global_stats.xml"))
                                inputStream.copyTo(outputStream)
                            }
                        }
                    }
                ...
                it.zipInputStream().use { inputStream ->
                    ...
                    while (entry != null) {
                        when (entry.name) {
                            ...
                            "icebeats_global_stats.xml" -> {
                                val parentFile = context.filesDir.parentFile
                                if (parentFile != null) {
                                    val destFile = parentFile / "shared_prefs" / "icebeats_global_stats.xml"
                                    destFile.parentFile?.mkdirs()
                                    destFile.outputStream().use { outputStream ->
                                        inputStream.copyTo(outputStream)
                                    }
    ...
    suspend fun backupToDrive(context: Context, email: String, name: String = "icebeats User"): com.valora.icebeats.utils.DriveResult<Boolean> {
        return try {
            ...
            tempFile.outputStream().use { fileOut ->
                fileOut.buffered().zipOutputStream().use { outputStream ->
                    ...
                    if (parentFile != null) {
                        ...
                        if (statsPrefsFile.exists()) {
                            statsPrefsFile.inputStream().buffered().use { inputStream ->
                                outputStream.putNextEntry(java.util.zip.ZipEntry("icebeats_global_stats.xml"))
                                inputStream.copyTo(outputStream)
                            }
                        }
                    }
                    ...
                    java.io.FileInputStream(database.openHelper.writableDatabase.path).use { inputStream ->
                        outputStream.putNextEntry(java.util.zip.ZipEntry(com.valora.icebeats.db.InternalDatabase.DB_NAME))
                        inputStream.copyTo(outputStream)
                    }
                }
            }
            ...
            if (success) {
                com.valora.icebeats.utils.DriveResult.Success(true)
            } else {
                com.valora.icebeats.utils.DriveResult.Error(Exception("Cloud backup upload failed"))
            }
        } catch (e: Exception) {
            e.printStackTrace()
            com.valora.icebeats.utils.DriveResult.Error(e)
        }
    }

    suspend fun restoreFromDrive(context: Context, email: String): com.valora.icebeats.utils.DriveResult<Boolean> {
        return try {
            ...
            if (!success) {
                return com.valora.icebeats.utils.DriveResult.Error(Exception("Backup not found in cloud"))
            }
                ...
                fileIn.zipInputStream().use { inputStream ->
                    ...
                    while (entry != null) {
                        when (entry?.name) {
                            ...
                            com.valora.icebeats.db.InternalDatabase.DB_NAME -> {
                                kotlinx.coroutines.runBlocking(kotlinx.coroutines.Dispatchers.IO) {
                                    database.checkpoint()
                                }
                                database.close()
    ...
    fun resetVisitorData(context: Context) {
        runCatching {
            // Implementa aqu• c•mo borras VISITOR_DATA, por ejemplo, desde DataStore
            ...
            Toast.makeText(
                context,
                "VISITOR_DATA reseteado. La aplicaci•n se reiniciar•.",
                Toast.LENGTH_SHORT
            ).show()
